...
class WelcomeDB(ezcord.DBHandler):
    ...
    async def enable(self, server_id, channel_id, enabled):
        async with self.start() as cursor:
            await cursor.exec("INSERT OR IGNORE INTO servers (server_id) VALUES (?)", server_id)
            await cursor.exec("UPDATE servers SET channel_id = ? WHERE server_id = ?", channel_id, server_id)
            await cursor.exec("UPDATE servers SET enabled = ? WHERE server_id = ?", enabled, server_id)
...
class WelcomeSystem(ezcord.Cog):
    ...
    async def setup(self, ctx: discord.ApplicationContext):
        ...
        if status == "Off":
            ...
            await ctx.respond(f"> **Bitte schalte das System erst mit {self.bot.get_cmd('welcome stop')} aus, und nutze dann diesen Command erneut!**", ephemeral=True)
